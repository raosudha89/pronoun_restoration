...
def extract_features(en, en_sents, zh, zh_sents, zh_pos, options):
    ...
    for l_en in en.readlines():
        ...
        if re.match("</DOC", l_en):
			...
			label = '2'
                    if label == 'none':
                        ...
                        total_label = label + 'v'
		    output = last_zh.replace("|", " ").replace(":", " ")
		    sample_wt = 1
		    if(total_label[-1] == 'v'):
			sample_wt = 2
		    last_zh_pos = last_zh_pos.replace("|", " ").replace(":", " ")
		    word_tag_list = last_zh_pos.split()
	            sent = last_zh
		    tags = ""
		    verbs = ""
		    pronouns = ""
		    ...
		    print "%s " % (to_searn_label(total_label)) + feature_set 
	    ...
	    en_sent = en_sents.readline()
	    en_sent = en_sent.rstrip("\n")
	    zh_sent = zh_sents.readline()
	    zh_sent = zh_sent.rstrip("\n")
		...
		last_zh_pos = l_zh_pos
                ...
                last_en = last_en + " " + en_sent
                #last_zh = last_zh + " " + m_zh.group(4)
                last_zh = last_zh + " " + zh_sent
...
def do_main(argv):
    if len(argv) < 4:
	print "usage: python to_seq_labeling_v1.py --en-file <file> --en-sent-file <file> --zh-file <file> --zh-sent-file <file> --zh-pos-file <file> --bow --f-count --s-count --zh-label --pos-tags --verbs --pronouns"
	return
    parser = OptionParser()
    parser.add_option("--en-file", dest="en_file", action="store",
                  help="write report to FILE", metavar="FILE")
    parser.add_option("--en-sent-file", dest="en_sent_file", action="store",
                  help="write report to FILE", metavar="FILE")
    parser.add_option("--zh-file", dest="zh_file", action="store",
                  help="write report to FILE", metavar="FILE")
    parser.add_option("--zh-sent-file", dest="zh_sent_file", action="store",
                  help="write report to FILE", metavar="FILE")
    ...
    en_sent_file = open(options.en_sent_file)
    zh_sent_file = open(options.zh_sent_file)
    extract_features(en, en_sent_file, zh, zh_sent_file, zh_pos,options)
    en.close()
    zh.close()
    zh_pos.close()
    en_sent_file.close()
